...
internal sealed class ZkDeviceClient : IDisposable
{
    const ushort CmdConnect = 1000, CmdExit = 1001, CmdDisable = 1003, CmdEnable = 1002;
    const ushort CmdPrepare = 1500, CmdData = 1501, CmdFree = 1502, CmdGetAttendance = 13, CmdPrepareBuffer = 1503, CmdUserTemp = 9;
    ...
    public List<AttendanceRecord> DownloadAttendance()
    {
        ...
        try
        {
            var prepared = Send(CmdGetAttendance, Array.Empty<byte>()); if (prepared.Command != CmdPrepare && prepared.Command != AckOk) throw new InvalidOperationException($"No se pudieron solicitar las marcaciones (respuesta {prepared.Command}).");
