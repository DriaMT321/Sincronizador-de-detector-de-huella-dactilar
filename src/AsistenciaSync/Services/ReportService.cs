...
public static class ReportService
{
    ...
    {
        using var sw = new StringWriter(CultureInfo.InvariantCulture); sw.WriteLine("sep=;"); sw.WriteLine("ID empleado;Nombre completo;Fecha;Ingreso 1;Salida 1;Ingreso 2;Salida 2;Horas trabajadas;Horas esperadas;Tiempo de tardanza;Salida anticipada;Tiempo faltante;Tiempo extra;Estado"); foreach (var r in rows) sw.WriteLine(Line(r.EmpleadoId, r.Nombre, r.Fecha.ToString("dd/MM/yyyy"), Time(r.Entrada), Time(r.Salida), Time(r.SecondEntry), Time(r.SecondExit), Hours(r.Trabajadas), Hours(r.Esperadas), Hours(r.MinutosTarde), Hours(r.MinutosSalidaAnticipada), Hours(r.MinutosFaltantes), Hours(r.MinutosExtra), r.Estado)); return sw.ToString();
    }
    ...
    sealed record DailyRow(string EmpleadoId, string Nombre, DateTime Fecha, DateTime? Entrada, DateTime? Salida, DateTime? SecondEntry, DateTime? SecondExit, int Trabajadas, int Esperadas, int MinutosTarde, int MinutosSalidaAnticipada, int MinutosFaltantes, int MinutosExtra, string Estado);
}
